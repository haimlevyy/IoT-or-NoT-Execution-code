...
class OnDemandDHCPClassifier:
    def __init__(self, oracle, dhcp_fdb):
        ...
        print len(self.dhcp_records)

    def classify_device(self, dev, sids, slot_width=1200):
        tn, fp, fn, tp = 0, 0, 0, 0
        ...
        probas = np.array([])
        dps = get_dev_sw_slots(self.dhcp_records, dev, 0)
        for sid in sids:
            labels = []
            X, y = [], []
            for dp in dps:
                if not is_record_in_width(dp, sid, slot_width):
                    continue
                labels += self.prepare_vector(dp.hostname, dp.vendor_class, dp.req_lst, dp.mds, dp.msg_t)
            if len(labels) == 0:
                probas = np.hstack((probas, [np.nan]))
                continue
            labels = self.encode(list(set(labels)))
            X.append(labels)
            y.append(int(dev.isiot))
            probas = np.hstack((probas, self.classifier.predict_proba(X)[0][1]))

        return probas

    def classify_on_demand(self, dev, slot_id, slot_width=1200):
        k = None
