...
class FolderLoader(object):
    ...
    def load_fold(self, is_lst=True):
        fold = Fold()
        fold.load_fold(self.train_path, self.test_path, is_lst)
        return fold
...
class Fold(object):
    ...
    def load_fold(self, train_path, test_path, is_lst):
    ...
    def get_dev_features(self, mac):
        for k in self.test.iotf.keys():
            if mac in k.replace("-", ":"):
                return self.test.iotf[k]
        for k in self.test.notf.keys():
            if mac in k.replace("-", ":"):
                return self.test.notf[k]
        for k in self.train.iotf.keys():
            if mac in k.replace("-", ":"):
                return self.train.iotf[k]
        for k in self.train.notf.keys():
            if mac in k.replace("-", ":"):
                return self.train.notf[k]
        return None
